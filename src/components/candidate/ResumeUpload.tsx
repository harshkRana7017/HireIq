...
const MOCK_PROFILES = [
  {
    ...
    skills:
      'Expert in React, TypeScript, Tailwind CSS, Vite, State Management and Cypress testing engines. Over 6 years leading frontend systems.'
  },
  {
    name: '🐍 Pure Backend template',
    fileName: 'jacob_django_postgres.pdf',
    skills:
      'Passionate Python developer skilled in FastAPI, SQL Postgres databases and Redis memory stores. Deep Docker and API engineering.'
  },
  {
    name: '🎨 Product Designer template',
    fileName: 'sophia_figma_ux.pdf',
    skills:
      'UX Researcher and UI Designer. Fluent in Figma design systems, prototyping, wireframes, and validating design tokens.'
  }
];
...
export const ResumeUpload: React.FC<ResumeUploadProps> = ({
  ...
  const selectPresetProfile = (
    preset: typeof MOCK_PROFILES[0]
  ) => {
    setCurrentFile({
    ...
    startUploadFlow(undefined, preset.skills, preset.fileName);
  };
  ...
  return (
    ...
    >
      <div
        ...
        className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all cursor-pointer flex flex-col items-center justify-center min-h-[220px] ${isDragging
      ...
      {uploadState === 'idle' && (
        <div className="border border-zinc-150 dark:border-zinc-805 rounded-2xl p-5 bg-white dark:bg-zinc-900/60 shadow-xs">
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-3 leading-normal">
            No real resume file around? Click our pre-packaged profiles below to
            see the platform match automatically!
          </p>
